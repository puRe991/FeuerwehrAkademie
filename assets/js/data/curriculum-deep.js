...
export const EXTRA_LESSONS_3 = {
  ...
  's-rettung': [
    {
      id: 's3', title: 'Tragehilfen & patientenschonender Transport', duration: 18,
      blocks: [
        H2('geraete', 'Geräte für den Transport'),
        TBL(['Gerät', 'Einsatz'],[
          ['Krankentrage', 'Standardtransport liegender Patienten'],
          ['Schleifkorbtrage', 'unwegsames Gelände, Höhen/Tiefen, Fixierung'],
          ['Spineboard', 'achsengerechte Rettung bei Wirbelsäulenverdacht'],
          ['Schaufeltrage', 'schonendes Aufnehmen ohne Umlagern'],
          ['Rettungstuch', 'kurze Strecken, enge Wege'],
        ]),
        H2('achse', 'Achsengerecht bewegen'),
        P('Bei Verdacht auf Wirbelsäulenverletzung wird die Person <b>achsengerecht</b> bewegt – Kopf, Hals und Rumpf bleiben in einer Linie. Das erfordert mehrere Helfer und einen Kommandogeber.'),
        CO('warn', 'Nur so viel bewegen wie nötig', 'Jede Bewegung eines Schwerverletzten birgt Risiken. Außer bei akuter Lebensgefahr (Sofortrettung) wird schonend, abgestimmt mit dem Rettungsdienst, gerettet.'),
        H2('team', 'Im Team heben'),
        KF(
          'Ein Kommandogeber zählt an („Achtung – heben!")',
          'Aus den Beinen heben, Last nah am Körper',
          'Gleichmäßig und synchron bewegen',
          'Patient ansprechen und über jeden Schritt informieren'),
      ],
    },
    {
      id: 's4', title: 'Rettung über Leitern & Rettungsgeräte', duration: 16,
      blocks: [
        H2('leiter', 'Rettung über tragbare Leitern'),
        P('Über Steck- oder Schiebleiter können Personen aus Obergeschossen gerettet werden – nur geübt, gesichert und mit ausreichend Helfern. Die Leiter wird während der Rettung festgehalten.'),
        CO('warn', 'Nur geübte Leiterrettung', 'Die Rettung einer hilflosen Person über eine Leiter ist anspruchsvoll und gefährlich. Sie erfordert Übung; im Zweifel Drehleiter oder Sprungrettungsgerät (nur als letztes Mittel).'),
        H2('dl', 'Drehleiter'),
        P('Die Drehleiter mit Korb ist das komfortabelste Rettungsmittel aus Höhen – für gehunfähige Personen, mit Krankentrage-Lagerung im Korb. Aufstellflächen freihalten, Standsicherheit (Abstützung) beachten.'),
        H2('sprung', 'Sprungrettungsgerät'),
        DEF('Sprungrettungsgerät (Sprungpolster)', 'Aufblasbares Polster, das eine springende Person auffängt. Nur allerletztes Mittel, wenn kein anderer Rettungsweg möglich ist – begrenzte Sprunghöhe, hohes Risiko.'),
        CO('tip', 'Reihenfolge der Rettungswege', 'Bevorzugt über bauliche Rettungswege (Treppe), dann Drehleiter/tragbare Leitern, dann Rettungsgeräte – das Sprungrettungsgerät ist die absolute Ausnahme.'),
      ],
    },
    {
      id: 's5', title: 'Menschenrettung planen: Prioritäten & viele Betroffene', duration: 16,
      blocks: [
        H2('prio', 'Prioritäten setzen'),
        P('Sind mehrere Menschen in Gefahr, muss die Führungskraft priorisieren. Grundsatz: unmittelbar Lebensbedrohte zuerst, dann akut Gefährdete, dann übrige.'),
        KF(
          'Akut vom Feuer/Rauch/Einsturz Bedrohte zuerst',
          'Rufende, sichtbare Personen leiten die Suche',
          'Systematisch absuchen, damit niemand vergessen wird',
          'Gerettete registrieren/melden (Vollzähligkeit)'),
        H2('manv', 'Massenanfall von Verletzten (MANV)'),
        DEF('Sichtung (Triage)', 'Bei vielen Betroffenen ordnet der Rettungsdienst die Patienten nach Behandlungsdringlichkeit (Sichtungskategorien). Die Feuerwehr unterstützt bei Rettung, Transport und Betreuung – der Rettungsdienst führt medizinisch.'),
        CO('info', 'Zusammenarbeit', 'Bei MANV arbeiten Feuerwehr, Rettungsdienst und Hilfsorganisationen eng zusammen. Klare Führung, Sammelplätze (Verletzten-/Behandlungsplatz) und geordneter Abtransport sind entscheidend.'),
        CO('tip', 'Ruhe bewahren', 'Viele Betroffene, laute Lage, Zeitdruck – gerade dann zählt strukturiertes, ruhiges Führen nach dem Führungsvorgang. Überblick behalten statt sich zu verzetteln.'),
      ],
    },
  ],
  ...
  't-wasserversorgung': [
    {
      id: 't3', title: 'Hydranten & Löschwassernetz', duration: 16,
      blocks: [
        H2('netz', 'Das Löschwassernetz'),
        P('Hydranten sind an das öffentliche Trinkwassernetz angeschlossen. Ein <b>Ringnetz</b> liefert mehr Wasser als ein Endstrang (Stichleitung), weil es von zwei Seiten gespeist wird.'),
        TBL(['Merkmal', 'Bedeutung'],[
          ['Ringleitung', 'höhere Ergiebigkeit, Speisung von zwei Seiten'],
          ['Stichleitung', 'begrenzte Ergiebigkeit, nur eine Seite'],
          ['Nennweite (DN)', 'je größer die Leitung, desto mehr Durchfluss'],
        ]),
        H2('kennung', 'Hinweisschilder'),
        DEF('Hydrantenschild', 'Weiß-rotes Schild (nach DIN) zeigt Lage (Abstand), Art und Nennweite des Hydranten an – so wird der nächste Hydrant schnell gefunden, auch unter Schnee/Laub.'),
        H2('ergiebig', 'Ergiebigkeit einschätzen'),
        P('Die entnehmbare Wassermenge hängt von Netzdruck und Leitungsdurchmesser ab. Die Objektkenntnis (welcher Hydrant liefert wie viel) ist Teil der Einsatzvorbereitung – bei Großobjekten oft im Feuerwehrplan vermerkt.'),
        CO('warn', 'Netz kann zu wenig liefern', 'Bei hoher Brandlast oder in ländlichen Gebieten reicht das Hydrantennetz oft nicht. Dann sind unabhängige Löschwasserquellen (Gewässer, Behälter) und Wasserförderung über lange Wegstrecke nötig.'),
      ],
    },
    {
      id: 't4', title: 'Offene Wasserentnahme im Detail', duration: 16,
      blocks: [
        H2('saugstelle', 'Die Saugstelle'),
        P('An offenen Gewässern muss eine geeignete Saugstelle gewählt werden: fester Untergrund, ausreichende Wassertiefe, sichere Zugänglichkeit für Fahrzeug und Mannschaft.'),
        STEPS(
          'Saugstelle erkunden (Tiefe, Untergrund, Zugang)',
          'Saugschläuche kuppeln, Dichtungen prüfen',
          'Saugkorb mit Ventil- und Halteleine sichern',
          'Saugleitung zu Wasser bringen (Saugkorb nicht auf Grund)',
          'Entlüften, ansaugen, Betriebsdruck einregeln'),
        DEF('Faltbehälter', 'Mobiler Löschwasserbehälter, der als Puffer dient – z. B. beim Pendelverkehr mit Tanklöschfahrzeugen: TLF füllen den Behälter, die Pumpe saugt daraus.'),
        CO('warn', 'Saugkorb frei halten', 'Der Saugkorb darf nicht auf den Grund absinken (Schlamm/Verstopfung) und nicht Luft ziehen (zu geringe Tiefe) – beides führt zu Kavitation und Förderabriss.'),
        H2('umwelt', 'Gewässerschutz'),
        P('Bei der Entnahme aus Gewässern auf Naturschutz achten (Fische am Saugkorb, Uferschäden vermeiden). Nach dem Einsatz Saugstelle sauber hinterlassen.'),
      ],
    },
    {
      id: 't5', title: 'Wasserförderung planen & berechnen', duration: 18,
      blocks: [
        H2('planung', 'Die lange Wegstrecke planen'),
        P('Für die Wasserförderung über lange Wegstrecke plant der Maschinist/Führer die Pumpenabstände so, dass an jeder Folgepumpe genug Eingangsdruck ankommt.'),
        KF(
          'Ausgangsdruck der Pumpe (z. B. 8–10 bar)',
        ...
        UL(
          ...
          '<b>Schlauchwagen/Schlauchtragekörbe</b> für lange B-Strecken',
          '<b>Pendelverkehr</b> mit TLF als Alternative im Gelände',
          '<b>Kommunikation</b> zwischen den Pumpen ist entscheidend'),
        CO('tip', 'Reserve einplanen', 'Immer mit Reserve planen: lieber eine Pumpe mehr in der Strecke als ein Förderabriss mitten im Löschangriff. „Ohne Wasser kein Angriff."'),
      ],
    },
  ],
